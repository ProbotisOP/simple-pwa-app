...
self.addEventListener('fetch', function (event) {
  ...
  if (applicationFiles.some((file) => path.endsWith(file))) {
    ...
    console.log(
      `Service worker forced to fetch from network. Url: ${event.request.url}`
    );
  ...
  event.respondWith(
    new Promise((resolve) => {
      ...
      getItem(resolvedPath).then((blob) => {
        if (!blob) {
          // Resource not found in offline storage. Fall back to network
          console.log(
            `Service worker unable to find in offline storage. Fallback to network. Url: ${event.request.url}`
          );
        ...
        // Determine content type based on file extension in the URL
        ...
        // Check for videos with URLs like the provided example
        if (url.includes('_jcr_content/renditions/original')) {
          contentType = 'video/mp4';
        }
        ...
        console.error(
          `Service worker unable to read item from offline storage. Error = ${JSON.stringify(
            e
          )}`
        );
